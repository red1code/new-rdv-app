...
import { ApprovedRendezvous } from 'src/app/models/rendezvous';
...
export class ApprovedRendezvousComponent implements OnInit {
  ...
  user!: User;
  // table variables
  approvedRDVs!: ApprovedRendezvous[];
  ...
  // popup variables
  rdv: ApprovedRendezvous | null = null;
  ...
  proceedToUpdate(data: ApprovedRendezvous) {
    this.rdvDateForInput = timeTolocalISOstring(new Date(data.rdvDate));
    this.rdv = data;
    this.showPopup = true
  }

  async updateRDV(data: ApprovedRendezvous | null, date: string) {
    if (!data) {
